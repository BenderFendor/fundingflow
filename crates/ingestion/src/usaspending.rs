...
impl Importer for UsaspendingImporter {
    ...
    async fn import(&self, pool: &PgPool) -> Result<ImportSummary> {
        ...
        if !response.status().is_success() {
            warn!("USASpending API returned status {}", response.status());
            summary.errors += 1;
            return Ok(summary);
        }

        let data: SpendingByAwardResponse = response.json().await?;
        info!(
            "USASpending: page {} with {} results",
            data.page_metadata.page,
            data.results.len(),
        );
        ...
        for award in &data.results {
            ...
            let agency_id = match Self::find_or_create_entity(
                pool,
                agency_name,
                EntityType::Agency,
                None,
            ...
            if let Err(e) = queries::create_award(
                pool,
                &CreateAwardRequest {
        ...
        info!(
            "USASpending import done: {} imported, {} errors",
